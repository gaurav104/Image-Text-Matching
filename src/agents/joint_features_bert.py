...
from graphs.models.model import Model, ModelBert, ModelBertOnly
from datasets.cuhk_pedes import CUHKLoader
from datasets.cuhk_pedes_bert import CUHKLoaderBert
...
class JointFeatureBert(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self.model = ModelBert(self.config)

        self.data_loader = CUHKLoaderBert(self.config)
...
#         param_other = list(param_other)
        ...
        parameters = [{'params': param_image, 'weight_decay': self.config.weight_decay}, 
        {'params': self.model.bilstm.bert_encoder.parameters(), 'weight_decay': self.config.weight_decay, 'lr':self.config.bert_learning_rate},
        {'params': self.model.bilstm.conv_text.parameters(), 'weight_decay': self.config.weight_decay},
        {'params': self.model.bilstm.bilstm.parameters(), 'weight_decay': self.config.weight_decay},
        {'params': param_id_classifier, 'weight_decay': self.config.weight_decay}]
#                      {'params': param_image_projection}, {'params': param_text_projection}]
        ...
        if self.config.optimizer=='Adam':
            ...
            self.optimizer = optim.AdamW(parameters, lr=self.config.learning_rate)


        
        
        # self.optimizer_2 = optim.SGD(parameters, lr=self.config.learning_rate, momentum=0.9)
#         
...
#             print_cuda_statistics()
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.config.max_epoch + 1):
            ...
            #     for param in self.model.bilstm.bert_encoder.parameters():
            #         param.requires_grad = True

            if not self.config.embed_non_trainable:
                for param in self.model.bilstm.best_encoder.parameters():
                    param.requires_grad = True
                
            

            self.model.train()
            self.model.bilstm.bert_encoder.eval()
...
#                 self.best_loss = R1_t2i
    ...
    def train_one_epoch(self):
        ...
        for step, (images, captions, labels, captions_length, attention_mask) in enumerate(data_loader):
            N,C,H,W = images.size()
            images, captions, labels = images.to(self.device), captions.to(self.device), labels.to(self.device)
            captions_length, attention_mask =  captions_length.to(self.device), attention_mask.to(self.device)
            self.optimizer.zero_grad()

            
            image_embeddings, text_embeddings = self.model(images, captions, captions_length, attention_mask)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            index = 0
            for step, (captions, labels,captions_length, attention_mask) in enumerate(self.data_loader.test_loader_caption):
                # images = images.cuda()
                ...
                text_embeddings = self.model.bilstm(captions, captions_length, attention_mask)
                # images_bank[index: index + interval] = image_embeddings
                text_bank[index: index + interval] = text_embeddings

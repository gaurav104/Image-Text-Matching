...
class Flowers(data.Dataset):
	...
	def __getitem__(self, index):
		...
		attention_mask, cap_length = self.attention_mask[index], self.cap_length[index]
...
class Flowers_captions(data.Dataset):
	'''
...
class Flowers_images(data.Dataset):
	'''
...
class FlowersLoaderBert:
	def __init__(self, config):
		...
		if self.config.run_on_cluster:
			output_bytes = subprocess.check_output("echo $SLURM_TMPDIR", shell=True)
			output_string = output_bytes.decode('utf-8').strip()
			image_dir = os.path.join(output_string, 'Flowers/images')
			anno_dir = os.path.join(output_string, 'Flowers/processed_data_bert')
		...
		train_set = Flowers('train', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							self.input_transform_train
						   )
		validation_set = Flowers('val', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)

		test_set = Flowers('test', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)
        
		test_set_image = Flowers_images('test', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)

		test_set_caption = Flowers_captions('test',
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)
...
if __name__ == '__main__':
	...
	train_set = Flowers('train', "/home/gaurav/Desktop/person-id/Datasets/Flowers/images","/home/gaurav/Desktop/person-id/Datasets/Flowers/processed_data_bert",50,transformations)

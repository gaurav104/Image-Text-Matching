...
class CUHK_pedes(data.Dataset):
	...
	def __init__(self, split, image_root, anno_root, max_length, transforms=None):
		...
		# if self.config.bert_embeddings:

			#tokenization and truncating captions
		self.padded_sequences = self.tokenizer([seq for seq in self.captions], padding=True)
		# self.padded_sequences['length'] = 

		self.captions = self.padded_sequences['input_ids']
		self.attention_masks = self.padded_sequences['attention_mask']
		self.attention_masks = [np.array(attention_mask, dtype=np.float32) for attention_mask in self.attention_masks]
		self.cap_length = [sum(attention_mask) for attention_mask in self.padded_sequences['attention_mask']]
		# else:
		# 	self.attention_mask = []
			# self.cap_length = []
	...
	def __getitem__(self, index):
		...
		attention_mask, cap_length = self.attention_masks[index], self.cap_length[index]
		# cap_length = self.cap_length[index]
		...
		return img, caption, label, cap_length, attention_mask
...
class CUHK_pedes_captions(data.Dataset):
	'''
	...
	def __getitem__(self, index):
		...
		caption, label =  self.captions[index], self.labels[index]
		attention_mask, cap_length = self.attention_masks[index], self.cap_length[index]
		...
		return caption, label, cap_length, attention_mask
...
class CUHK_pedes_images(data.Dataset):
	'''
...
class CUHKLoaderBert:
	def __init__(self, config):
		self.config = config

		jf = self.config.jitter_factor
		...
		if self.config.run_on_cluster:
			output_bytes = subprocess.check_output("echo $SLURM_TMPDIR", shell=True)
			output_string = output_bytes.decode('utf-8').strip()
			image_dir = os.path.join(output_string, 'CUHK-PEDES/images')
			anno_dir = os.path.join(output_string, 'CUHK-PEDES/processed_data_bert')
		...
		self.input_transform_train = transforms.Compose([transforms.RandomHorizontalFlip(),
														...
														transforms.ColorJitter(brightness=jf, contrast=jf, saturation=jf, hue=jf),
        												transforms.ToTensor(),
		...
		train_set_image = CUHK_pedes_images('train', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform_train)

		train_set_caption = CUHK_pedes_captions('train', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length)
        
		test_set_image = CUHK_pedes_images('test', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)

		test_set_caption = CUHK_pedes_captions('test',
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length)
...
# 			if len(self.config.modality)==1:
		...
		self.train_image_iteration = (len(train_set_image) + self.config.batch_size) // self.config.batch_size
		self.train_caption_iteration = (len(train_set_caption) + self.config.batch_size) // self.config.batch_size
		...
		self.train_loader_image = DataLoader(train_set_image, batch_size=self.config.batch_size, shuffle=True,
										   num_workers=self.config.data_loader_workers,
										   pin_memory=self.config.pin_memory)
		self.train_loader_caption = DataLoader(train_set_caption, batch_size=self.config.batch_size, shuffle=True,
										   num_workers=self.config.data_loader_workers,
										   pin_memory=self.config.pin_memory)

		self.test_loader_image = DataLoader(test_set_image, batch_size=self.config.batch_size, shuffle=False,
										   num_workers=self.config.data_loader_workers,
										   pin_memory=self.config.pin_memory)
		self.test_loader_caption = DataLoader(test_set_caption, batch_size=self.config.batch_size, shuffle=False,
										   num_workers=self.config.data_loader_workers,
										   pin_memory=self.config.pin_memory)
...
if __name__ == '__main__':
	...
	train_set = CUHK_pedes('train', "/home/gaurav/Desktop/person-id/Datasets/CUHK-PEDES/images","/home/gaurav/Desktop/person-id/Datasets/CUHK-PEDES/processed_data_bert",50,transformations)
	...
	# for ep in range(4):
	for step, (image, caption, label, length, attention_mask) in enumerate(train_loader):

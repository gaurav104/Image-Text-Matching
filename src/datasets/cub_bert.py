...
class CUB_captions(data.Dataset):
	'''
...
class CUBLoaderBert:
	def __init__(self, config):
		...
		if self.config.run_on_cluster:
			output_bytes = subprocess.check_output("echo $SLURM_TMPDIR", shell=True)
			output_string = output_bytes.decode('utf-8').strip()
			image_dir = os.path.join(output_string, 'CUB/images')
			anno_dir = os.path.join(output_string, 'CUB/processed_data_bert')
		...
		test_set_image = CUB_images('test', 
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)

		test_set_caption = CUB_captions('test',
							self.config.image_dir,
							self.config.anno_dir,
							self.config.max_length,
							transforms=self.input_transform)
...
if __name__ == '__main__':
	...
	train_set = CUB('train', "/home/gaurav/Desktop/person-id/Datasets/CUB/images","/home/gaurav/Desktop/person-id/Datasets/CUB/processed_data_bert",50,transformations)
